...
- **Historical developer activity** has no backtest-safe open source: CoinGecko's `/history` endpoint returns `developer_data` as all-null, and falling back to *current* dev data would leak the future into backtests. `get_dev_activity` therefore degrades gracefully (Phase 2). A keyless **GitHub API adapter** (commit/PR counts in a date window for the asset's repo) is the realistic path to real as-of dev activity — flagged as a follow-up, not taken in this change.
- **Structured output on cheap OpenRouter models**: the Phase 3 end-to-end run showed `qwen/qwen3.5-flash-02-23` and `qwen/qwen3-235b-a22b-2507` do not reliably serve structured output through OpenRouter (`tool_choice` 404 on the Trader; null structured result on the Portfolio Manager). The framework's free-text fallback (`invoke_structured_or_freetext`) handled both and the run still produced a valid five-tier rating — but for clean structured runs the deep tier should use a model with proven structured-output support. This is an operational model-selection note, not a code change.
