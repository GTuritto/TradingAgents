...
- Add a **crypto market-data vendor** (CCXT-based OHLCV + technical indicators) registered through the existing `data_vendors` / `tool_vendors` abstraction. Equity vendors (yfinance, alpha_vantage) remain untouched. The CCXT exchange is config-selectable via `ccxt_exchange`, defaulting to **Gemini**.
- Make data sources a **config-driven registry**: each source (exchange, on-chain provider, news provider) is a named adapter, and adding or switching a source is a configuration change rather than a code edit to `dataflows/interface.py`.
...
- `crypto-data-layer`: CCXT-backed market-data vendor (OHLCV, technical indicators) with a config-selectable exchange, plus on-chain data providers, all exposed through a config-driven source registry.
...
- **Modified code:** `dataflows/interface.py` (`TOOLS_CATEGORIES` registration + source registry); `default_config.py` (`data_vendors` crypto options, new `ccxt_exchange` key defaulting to `gemini`, BTC `benchmark_map` default); `graph/setup.py` (analyst selection); `graph/reflection.py` and `trading_graph.py` `_fetch_returns` / `_resolve_benchmark` (24/7 calendar, BTC baseline).
